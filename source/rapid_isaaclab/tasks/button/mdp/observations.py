...
def rel_ee_button_distance(env: ManagerBasedRLEnv) -> torch.Tensor:
    """The distance between the end-effector and the object."""
    ee_tf_data: FrameTransformerData = env.scene["ee_frame"].data
    button_tf_data: FrameTransformerData = env.scene["button_frame"].data

    return button_tf_data.target_pos_w[..., 0, :] - ee_tf_data.target_pos_w[..., 0, :]
