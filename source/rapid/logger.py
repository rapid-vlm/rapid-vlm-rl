...
class MetersGroup(object):
    def __init__(self, file_name, formating, save_csv=False):
    ...
    def _dump_to_csv(self, data):
        if self._csv_writer is None:
            # Build a stable header from the known formatting plus the current data and 'step'
            known_keys = [k for (k, _, _) in self._formating]
            fieldnames = sorted(set(list(data.keys()) + known_keys + ['step']))
...
class Logger(object):
    ...
    def dump(self, step, save=True, ty=None):
        ...
        # Ensure TB events are flushed so scalars appear immediately in the dashboard
        try:
