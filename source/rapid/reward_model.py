"""
Reward model ensemble for RAPID, built on the preference-based learning
foundation from RL-VLM-F (Wang et al., 2024).
"""
import numpy as np
import torch
...
from .vlms.openrouter_infer import (
    openrouter_query_two_images_free_form,
    openrouter_query_two_images_single_prompt,
)
...
class RewardModel:
    def __init__(self, ds, da, 
                ...
                # vlm related params
                vlm_label=True,
                env_name="isaaclab_cabinet_drawer_open",
        ...
        self.vlm_single_prompt = bool(vlm_single_prompt)
        
        
        file_path = os.path.abspath(__file__)
        dir_path = os.path.dirname(file_path)
        if cached_label_path is not None and str(cached_label_path).strip() != "":
            self.cached_label_path = os.path.join(dir_path, str(cached_label_path))
            self.read_cache_idx = 0
            if os.path.isdir(self.cached_label_path):
    ...
    # internal: wrap iterable with tqdm progress bar if enabled
    def _progress(self, iterable, total=None, desc=""):
        if getattr(self, 'progress_bar', False):
            try:
    ...
    def image_encoder_features(self, images, batch_size=128):
        """Encode HWC uint8 images with the current reward-model ensemble.

        Each ensemble member's pre-output representation is L2-normalized. The
        three representations are concatenated and scaled by ``sqrt(E)`` so the
        combined feature also has unit L2 norm. Models are temporarily put in
        evaluation mode and their original training modes are restored.
        """
        if not self.image_reward:
            raise RuntimeError("Reward-encoder features require image_reward=True")
        if self.resnet:
            raise NotImplementedError(
                "Reward-encoder FPS currently supports the RAPID CNN reward model, not ResNet"
            )
        if not self.ensemble:
            raise RuntimeError("Reward-model ensemble is empty")
        ...
        training_modes = [bool(model.training) for model in self.ensemble]
        output_batches = []
        ...
        try:
            ...
            with torch.no_grad():
                for start in range(0, count, batch_size):
                    batch_np = np.stack(
                        [np.asarray(image) for image in images[start : start + batch_size]],
                        axis=0,
                    )
                    if batch_np.ndim != 4 or batch_np.shape[-1] != 3:
                        raise ValueError(
                            "Reward-encoder images must have shape [N, H, W, 3]; "
                            f"received {batch_np.shape}"
                        )

                    model_device = next(self.ensemble[0].parameters()).device
                    batch = torch.as_tensor(batch_np, device=model_device)
                    ...
                    for model in self.ensemble:
                        if not hasattr(model, "encode"):
                            raise TypeError(
                                "Reward-encoder FPS requires ensemble models with an encode() method"
                            )
                        feature = model.encode(batch).reshape(batch.shape[0], -1)
                        feature = F.normalize(feature, p=2, dim=1, eps=1e-8)
                        member_features.append(feature)

                    combined = torch.cat(member_features, dim=1)
                    combined = combined / math.sqrt(float(len(member_features)))
                    output_batches.append(combined.cpu().numpy().astype(np.float32, copy=False))
        finally:
            for model, was_training in zip(self.ensemble, training_modes):
    ...
    def get_queries(self, mb_size=20):
        ...
        if 'Cloth' not in self.env_name:
            # Robustness: allow len_traj == size_segment (e.g., single-step image pools)
            max_start = max(1, int(len_traj - self.size_segment))
            random_idx_2 = np.random.choice(max_start, size=mb_size, replace=True).reshape(-1, 1)
            time_index_2 = time_index + random_idx_2
            random_idx_1 = np.random.choice(max_start, size=mb_size, replace=True).reshape(-1, 1)
    ...
    def get_label(self, sa_t_1, sa_t_2, r_t_1, r_t_2, img_t_1=None, img_t_2=None):
        ...
        if self.vlm_label:
            ...
            # number of candidate pairs (excluding identical-image pairs)
            attempted_pairs_count = int(np.sum(np.array(useful_indices) == 1))

            # Optional debug signal: if attempted_pairs_count is 0, all sampled pairs were visually identical.
            if os.environ.get("OPENROUTER_DEBUG", "0") == "1":
                try:
                    print(
                        f"[VLM][DEBUG] env={self.env_name} total_pairs={len(useful_indices)} "
                        f"attempted_pairs(excluding-identical)={attempted_pairs_count}"
                    )
                except Exception:
                    pass
                        
            if self.vlm == "openrouter_gemma3":
                # OpenRouter Gemma 3 inference: either two-stage (analysis+summary) or single-prompt
                import concurrent.futures
                import threading

                err_once = threading.Event()
                ...
                def process_pair(pair):
                    ...
                    try:
                        ...
                        PILImage.fromarray(img1).save(f1_name)
                        PILImage.fromarray(img2).save(f2_name)
                        f1.close()
                        f2.close()
                        
                        model_id = os.environ.get("OPENROUTER_MODEL", "google/gemma-3-12b-it")
                        if self.vlm_single_prompt:
                            # Single prompt path using centralized combined prompt from prompt.py
                            combined_prompt = openrouter_single_query_env_prompts.get(
                                self.env_name, ""
                            )
                            res = openrouter_query_two_images_single_prompt(
                                f1_name,
                                f2_name,
                                combined_prompt,
                                model=model_id,
                            ...
                            # Two-stage path (default)
                            res = openrouter_query_two_images_free_form(
                        ...
                        res = -1
                        # Debug once per labeling batch.
                        if os.environ.get("OPENROUTER_DEBUG", "0") == "1" and (not err_once.is_set()):
                            try:
                                err_once.set()
                                print(f"[OpenRouter][ERROR] VLM call failed: {type(e).__name__}: {e}")
                            except Exception:
                                pass
                    finally:
                        if f1_name and os.path.exists(f1_name):
                            try:
                                os.remove(f1_name)
                            except Exception:
                                pass
                        if f2_name and os.path.exists(f2_name):
                            try:
                                os.remove(f2_name)
                            except Exception:
                                pass
                ...
                pairs = list(zip(img_t_1, img_t_2))
                # Parallelize VLM calls
                # Allow user to control parallelism via env var; default to 10
                try:
                    max_workers = int(os.environ.get("VLM_PARALLEL_REQUESTS", "10"))
                except Exception:
                    max_workers = 10
                
                with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                    vlm_labels = list(executor.map(process_pair, pairs))
            ...
            acc = 0.0
            accepted_count = int(len(vlm_labels))
            if accepted_count > 0:
                acc = float(np.sum(vlm_labels == rational_labels) / accepted_count)
                # Reduce noise: print once and only if progress bar is disabled (trainer may show its own progress)
                if not getattr(self, 'progress_bar', False):
                    print(f"vlm labels accepted: {accepted_count}/{attempted_pairs_count}, acc vs rational: {acc:.3f}")
            else:
                if not getattr(self, 'progress_bar', False):
                    print("no vlm label")
                # Even when progress bars are enabled, surface a short reason if debugging.
                if os.environ.get("OPENROUTER_DEBUG", "0") == "1":
                    try:
                        if attempted_pairs_count <= 0:
                            print("[VLM][DEBUG] 0 accepted because all sampled image pairs were identical (diff < 1e-3).")
                        else:
                            print(
                                "[VLM][DEBUG] 0 accepted even though images differed; likely OpenRouter errors, timeouts, "
                                "or model responses not parseable into LABEL/0/1." 
                                "Enable OPENROUTER_DEBUG=1 to see the first error above."
                            )
                    except Exception:
                        pass
